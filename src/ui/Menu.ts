...
export class Menu {
	 ...
	 * 添加按钮
	 */
	private addButton(button: Omit<MenuButton, 'hovered'>): void {
		this.buttons.push({ ...button, hovered: false });
	}
	...
	private renderButtons(): void {
		...
		for (const button of this.buttons) {
			// 按钮背景
			if (button.hovered) {
				ctx.fillStyle = '#3b82f6'; // 悬停时的蓝色
			} else {
				ctx.fillStyle = '#1e293b'; // 默认深色
			}
			ctx.fillRect(button.x, button.y, button.width, button.height);

			// 按钮边框
			ctx.strokeStyle = button.hovered ? '#60a5fa' : '#475569';
			ctx.lineWidth = 3;
			ctx.strokeRect(button.x, button.y, button.width, button.height);
